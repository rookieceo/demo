...
class PostControllerTest {
	...
	final void testList() throws Exception {
		...
		this.mockMvc
				.perform(MockMvcRequestBuilders.get("/api/posts").param("page", "2").param("size", "15")
						.param("sortBy", "createdAt").param("direction", "DESC"))
	...
	final void testCreate() throws Exception {
		...
		MvcResult result = this.mockMvc
				.perform(MockMvcRequestBuilders.post("/api/posts").contentType(MediaType.APPLICATION_JSON)
						.content(this.objectMapper.writeValueAsString(dto)))
				.andExpect(MockMvcResultMatchers.status().isOk()).andReturn();

		PostEntity postEntity = this.objectMapper.readValue(result.getResponse().getContentAsString(),
				PostEntity.class);
	...
	final void testUpdate() throws Exception {
		...
		MvcResult result = this.mockMvc
				.perform(MockMvcRequestBuilders.put("/api/posts/" + dto.getPostId())
...
//
	...
	final void testDelete() throws Exception {
		...
		// 삭제처리
		this.mockMvc
				.perform(MockMvcRequestBuilders.delete("/api/posts/" + postId).contentType(MediaType.APPLICATION_JSON))
				.andExpect(MockMvcResultMatchers.status().isOk());
	...
	@Test
	public void testListUsingQuerydsl() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.get("/api/posts-querydsl").contentType(MediaType.APPLICATION_JSON))
				.andExpect(MockMvcResultMatchers.status().isOk());
	}
